# Deterministic second-pass refinement schedule for the median (LAD) log-variance
# driver: around every pass-one arg-extremum and every verified crossing witness,
...
# can only sharpen the attained hull, never lose an earlier value. Reused on the
# same cache and budget_state across the driver's two passes. Definitions only;
# sourced by log_var_eq_lad.R's constants (logvar_lad_refine_radii) plus the
# engine's logvar_b_key; consumed by the driver.
