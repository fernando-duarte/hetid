...
# and Plan 7 estimator object (math, recession, solver sourced below).
source("scripts-paper/log_var_eq_harvey_math.R")
source("scripts-paper/log_var_eq_harvey_recession.R")
source("scripts-paper/log_var_eq_harvey_solver.R")
source("scripts-paper/log_var_eq_harvey_tools.R")

# The Plan 7 estimator object. Callable only after the driver stability
...
logvar_harvey_estimator <- function(w1, w2, pcr, qtr, b_point = NULL,
  ...
  metadata <- list(
    ...
    inner_solver = "Fisher scoring + backtracking", response_scale = "variance",
    ...
    spec_id = logvar_harvey_spec_id(list(
      ...
      line_search_halvings = 30L, maxit = 200L,
