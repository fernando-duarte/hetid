...
# Deterministic given boot_seed. Writes draws RDS + diagnostics CSV via run_all.R.
source("scripts/utils/stats_utils.R") # mbb_index (already loaded upstream)
source("scripts/utils/logvar_set_envelope.R") # envelope + robust_scale/boot_min_reps
source("scripts/utils/logvar_set_bootstrap_core.R") # prepare/draw/collect
stopifnot(is.finite(boot_reps), boot_reps >= 2L)
# Builders reuse the frozen PPML response_scale + Harvey log-OLS start (harvey_sets.R).
...
utils::write.csv(
  ...
  file.path(out_dir, "log_var_eq_set_inference_diagnostics.csv"),
  row.names = FALSE
)
saveRDS(
  list(
  ...
  file.path(out_dir, "log_var_eq_set_boot_draws.rds")
)
cat(sprintf(
