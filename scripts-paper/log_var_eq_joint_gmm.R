# Gate dispatch, the pure joint_gmm_spec core, and the Stage A graph-replication
# check for the joint moment-compatibility layer (Plan 4, logvar-joint-gmm).
# Definitions only. The enabled-block and Stage C predicates read the validated
...
# test_logvar_joint_gmm.R and, at pipeline time, by run_all.R.

source("scripts-paper/log_var_eq_joint_artifacts.R")
source("scripts-paper/log_var_eq_joint_projection.R")
...
# no-answer default, which runs only Stage A plus graph replication).
...
# Stage C is enabled only with an explicit request and a nonempty ratified delta
...
# Stage A graph replication: at every feasible off-crossing b, the just-identified
...
source("scripts-paper/log_var_eq_joint_gmm_driver.R")
