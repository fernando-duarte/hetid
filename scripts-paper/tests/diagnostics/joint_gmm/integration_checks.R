# Integration checks: gate-branch dispatch over the committed decision record,
...
# committed helpers plus the driver, identity, budget, and artifact contracts.
# Sourced by test_joint_gmm.R, which owns check(),
...
# scientific choices and recomputing the spec ID with the decision helpers.
...
# Gate branch matrix (dossier section 11): enable flags map to moment blocks; ratified-projection
...
# block, a ratified-projection delta (even without the request flag), or a non-a_L target fails.
...
# Language guard: the comparison note
...
# Ratified projection is gated: an empty ratified delta returns the disabled
# reason, never a region.
