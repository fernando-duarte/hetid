# graph-replication moment-layer checks: graph equivalence, residualized-z redundancy and
...
# Jacobian rank, and Jensen gap live in the companion profile_checks.R
# (split to stay under the repository line cap). Sourced by test_joint_gmm.R,
# which owns check(), jg_try, jg_need, and the jg_fx fixtures. Every check first asserts
# each required symbol exists, so an absent module fails closed for the right reason
...
# graph-replication rank precondition: a rank-deficient design stops before projection.
