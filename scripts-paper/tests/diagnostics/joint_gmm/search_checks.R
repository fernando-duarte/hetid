# Compatibility-search checks: the pinned constants block, complete candidate construction,
...
# test_joint_gmm.R, which owns check(), jg_try, jg_need, and jg_fx. Every check
# asserts the required symbol exists first, so an absent module fails closed for
...
# Tau nesting: the hard-stop-with-floor carries the prior arg-min when a
