# Thin driver for the base-R log-variance dynamics gate (Plan 5, Task 2). It
...
# mirrors the Plan 1/4 driver idiom so the file defines nothing and errors not.
#
# Wired into run_all.R after set_id_mean_eq and log_var_eq are built (so the
# objects exist) and after log_var_eq_joint_gmm_run.R. No decision-file
# reference here -- the router and approvals are Task 3.
...
if (exists("log_var_eq") && exists("set_id_mean_eq")) {
  ...
  dyn_out <- if (exists("out_dir")) out_dir else "scripts-paper/output"
  dir.create(dyn_out, showWarnings = FALSE, recursive = TRUE)
  dyn_gate_rds <- file.path(dyn_out, "log_var_eq_dynamics_gate.rds")
  dyn_status_rds <- file.path(dyn_out, "log_var_eq_egarch_status.rds")
