...
# the base-R log-variance dynamics gate (Plan 5, Task 2). Definitions only;
# sourced by log_var_eq_dynamics_gate_core.R after the gate primitives, and it
...
# whether a downstream approval is now pending, and the gate-record path. Task 3
# rewrites this on every terminal routing branch.
logvar_gate_status_manifest <- function(record) {
  ...
  list(
    ...
    gate_record_path = "scripts-paper/output/log_var_eq_dynamics_gate.rds"
  )
}

# the descriptive sensitivity set over representative feasible b_N witnesses
source("scripts-paper/log_var_eq_dynamics_gate_sensitivity.R")
