...
logvar_lad_console_block <- function(lad, taus) {
  ...
  for (i in seq_along(keys)) {
    ...
    cat(sprintf(
      ...
      d$counters[["scan"]], d$counters[["probe"]], au$nonunique$n_probed,
      d$counters[["polish"]], d$counters[["cold_start"]], d$counters[["cache_hit"]],
      d$n_evaluated, d$n_failed
    ))
    cat(sprintf(
      "    coverage: %d witnesses, %d flagged, %d unresolved, %d paths; min|eps| %.3g\n",
