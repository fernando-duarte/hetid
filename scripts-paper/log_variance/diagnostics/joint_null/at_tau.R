...
# replication failure.
.jn_replication <- function(
  b_hat, q_hat, records, w1, w2, proj, d_inv2, qs, root_tol, control
) {
  scale <- .derive_theta_scale(qs)
  omega <- .derive_constraint_scales(qs, scale)
  tol_b <- control$coordinate_agreement_rel_tol * pmax(scale, abs(b_hat))
  agree <- 0L
  basins <- 0L
  max_scaled <- 0
  for (rec in records) {
  ...
  list(
    agree = agree >= control$required_agreeing_starts,
    n_agreeing = agree, basin_count = 1L + basins,
...
logvar_joint_null_at_tau <- function(tau, tau_display, b_tab, w1, w2, proj, d_inv2,
  ...
  scan <- logvar_joint_null_scan(
    qs, b_tab, w1, w2, proj, d_inv2, control$grid_n,
    prior_minima = prior_minima, seed_points = list(), control = control
  )
  polish <- logvar_joint_null_polish(
    qs, scan$starts, w1, w2, proj, d_inv2, control
  )
  b_hat <- scan$best$b
  ...
  n_active <- sum(abs(e_hat) <= control$crossing_rel_tol * e_scale_ref)
  apparent <- is.finite(scaled_linf) && scaled_linf <= root_tol
  mach_adj <- min_abs_e <= control$machine_adjacent_rel_tol * e_scale_ref
  status <- "bounded"
  ...
  if (isTRUE(scan$sparse_grid)) {
    ...
    stab <- logvar_joint_null_stability(
      b_hat, w1, w2, proj, qs, eps_ref, root_tol, control
    )
    rep <- .jn_replication(
      b_hat, q_hat, polish$records, w1, w2, proj, d_inv2, qs,
      root_tol, control
    )
    o2 <- logvar_joint_null_objective(b_hat, w1, w2, proj, d_inv2)
    fresh_ok <- identical(as.numeric(o2$q), q_hat) &&
      max(abs(as.numeric(o2$s) - slopes)) <=
        control$fresh_map_rel_tol * max(1, max(abs(slopes)))
    ...
    if (.jn_is_witness(
      stab, rep, scaled_linf, feas, mach_adj, e_hat, fresh_ok,
      root_tol, control
    )) {
  ...
  box_active <-
    any(abs(b_hat - lower) <= control$box_active_rel_tol * span) ||
      any(abs(b_hat - upper) <= control$box_active_rel_tol * span)
  ...
  structure(.jn_assemble_row(c(
    list(
      tau_display = as.numeric(tau_display),
      tau_internal = as.numeric(tau),
      scaled_l2 = scaled_l2,
      scaled_linf = scaled_linf
    ),
    .jn_parameter_fields(b_hat, theta, scales),
    list(
      ...
      argmin_source = argmin_source,
      winning_start_type = .jn_win_type(win_type),
      n_agreeing_starts = n_agree,
      start_agreement_max_scaled = agree_max,
      ...
      l2_rank_under_linf = linf$l2_rank,
      linf_rank_under_l2 = linf$linf_rank,
