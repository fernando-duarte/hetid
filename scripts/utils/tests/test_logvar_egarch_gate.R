# Offline checks for the base-R log-variance dynamics gate (Plan 5, Task 2): the
...
# reference in any gate file). A thin entry point mirroring test_logvar_harvey.R;
# run from the worktree root:
#   Rscript scripts/utils/tests/test_logvar_egarch_gate.R
...
source(here::here("scripts-paper/log_var_eq_dynamics_gate_core.R"))
...
gate_files <- c(
  "scripts-paper/log_var_eq_dynamics_gate_core.R",
  "scripts-paper/log_var_eq_dynamics_gate_record.R",
  "scripts-paper/log_var_eq_dynamics_gate_sensitivity.R",
  "scripts-paper/log_var_eq_dynamics_gate.R",
  "scripts-paper/log_var_eq_egarch_cleanup.R"
)
gate_files <- gate_files[file.exists(vapply(gate_files, here::here, character(1)))]
has_dep <- any(vapply(gate_files, function(f) {
  grepl_any(readLines(here::here(f)), "rugarch")
}, logical(1)))
