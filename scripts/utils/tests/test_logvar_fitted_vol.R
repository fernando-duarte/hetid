...
#   Rscript scripts/utils/tests/test_logvar_fitted_vol.R
...
source(here::here("scripts-paper/log_var_eq_fitted_vol_adapter.R"))
source(here::here("scripts-paper/log_var_eq_fitted_vol_envelope.R"))
source(here::here("scripts-paper/log_var_eq_fitted_vol_plot.R"))
...
paths <- vapply(
  c("ppml", "harvey"),
  function(estimator) logvar_fitted_vol_path("output", estimator),
  character(1)
)
check(
  ...
  identical(
    ...
    file.path(
      "output",
      c(
        "log_var_eq_fitted_volatility_ppml.pdf",
        "log_var_eq_fitted_volatility_harvey.pdf"
      )
    )
  )
)
