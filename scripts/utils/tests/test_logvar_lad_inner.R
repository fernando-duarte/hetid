...
# (scripts-paper/log_var_eq_lad_fit.R and log_var_eq_lad.R). A standalone entry
# point mirroring test_logvar_harvey.R: source the map/engine/log-OLS layers,
# source whichever LAD modules exist, define check() plus the quantreg-gated
# qr_check() wrapper, then run the inner checks. RED until Task 3 lands: the LAD
# modules are absent, so every module-dependent check fails closed through
# qr_check()'s tryCatch; the pure br-vs-LP and br-vs-fn oracles use quantreg
...
# root:
#   Rscript scripts/utils/tests/test_logvar_lad_inner.R
#
# Interface assumptions surfaced for the implementer (Task 3 freezes the names):
#   logvar_lad_fit_response(z, x_mat) returns the Plan 7 contract list with a
...
# The LAD modules do not exist yet; source only what is present so the suite
# runs to completion with module-dependent checks failing closed.
lad_modules <- c(
  "scripts-paper/log_var_eq_lad.R",
  "scripts-paper/log_var_eq_lad_fit.R"
)
for (f in lad_modules) if (file.exists(here::here(f))) source(here::here(f))
...
# The response fit reproduces the LP vertex through the Plan 7 contract.
...
# The response fit carries the full Plan 7 contract: a named length-5 coef, the
# non-comparable LAD objective, score_norm = NA, no warm start, and the slim
# diagnostics list holding the nonuniqueness flag.
qr_check("lad fit_response returns the Plan 7 contract fields", {
