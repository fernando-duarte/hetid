...
# (scripts-paper/log_var_eq_lad_gate.R). Quantreg-free by construction: package
...
# presence as approval. This file never skips. RED until Task 4 lands the base-R
# reader; every check runs inside gate_try and starts by asserting the reader
# exists, so an absent module fails the check closed rather than passing on the
# wrong error. Run from the worktree root:
#   Rscript scripts/utils/tests/test_logvar_lad_gate.R
#
# Interface assumption surfaced for the implementer (File structure names the
# module log_var_eq_lad_gate.R; the reader name is the plausible
...
gate_module <- "scripts-paper/log_var_eq_lad_gate.R"
if (file.exists(here::here(gate_module))) source(here::here(gate_module))
...
# Fail a check closed when the not-yet-defined reader errors or is absent.
