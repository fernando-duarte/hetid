...
#' pc1..pc4 basis, and any full-rank subset of rows returns the same weights.
#' It is recomputed here instead of being frozen as a literal because a literal
#' silently goes stale the moment the upstream import re-estimates the PCs.
#' That is exactly what happened at 0d9d812 ("Import variables dataset verbatim
#' from macro_dynamics"): the pre-import weights survived and the baseline
#' instrument they build correlated 0.36 with the VFCI it claimed to reproduce.
#' Coverage of the identified set is instrument-specific, so that is not a
#' labelling slip -- the sets no longer answer the VFCI specification.
#'
...
#' change no result; the raw weights are used so the combined instrument
#' PC %*% gamma literally IS VFCI - mean(VFCI) rather than a rescaled copy (the
#' PCs are kept at native scale; see create_data.R).
