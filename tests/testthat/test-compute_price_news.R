test_that("compute_price_news returns time series of length n-1", {
  ...
  # Test for maturity 5
  price_news_5 <- compute_price_news(test_env$yields, test_env$term_premia, i = 5)

  expect_type(price_news_5, "double")
  expect_length(price_news_5, nrow(test_env$yields) - 1)
...
test_that("compute_price_news works for maturity 1", {
  ...
  price_news_1 <- compute_price_news(test_env$yields, test_env$term_premia, i = 1)

  expect_type(price_news_1, "double")
  expect_length(price_news_1, nrow(test_env$yields) - 1)
...
test_that("yield news equals negative of price news", {
  ...
  # Test for maturity 4
  price_news_4 <- compute_price_news(test_env$yields, test_env$term_premia,
    i = 4,
    return_yield_news = FALSE
  )
  yield_news_4 <- compute_price_news(test_env$yields, test_env$term_premia,
    i = 4,
...
test_that("price news is negatively correlated with yield changes", {
  ...
  # Test for maturity 3
  i <- 3
  price_news_3 <- compute_price_news(test_env$yields, test_env$term_premia, i = i)
  yield_changes_3 <- diff(test_env$yields[[paste0("y", i)]]) / 100 # Convert to decimal
