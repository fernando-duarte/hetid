# Shared tau=0 mean-equation DGP: E[e1 e2 | z] = 0 by construction, so the tau=0
# point exists and recovers theta_true; x_var is the log-variance tests' design
simulate_tau0_dgp <- function(t_obs = 400, seed = 42) {
  ...
  # a separate volatility design: in the application the mean equation (SDF PCs) and the volatility
  # equation (lagged return PCs) use different regressors, so the fixtures keep them distinct too
...
# multiplicative chi-square response on exp(eta), the shape the log-variance equation feeds
# fit_log_variance; shared by the log-variance fitting, solver and standard-error tests
