...
make_w2_fixture <- function() {
  ...
  merged <- merge(
    ...
    acm[, c("yq", paste0("y", mats), paste0("tp", mats))],
    by = "yq"
  )
...
test_that("impose_b_zero = TRUE yields literal W2 = Y2 with zero coefficients", {
  ...
  fit <- compute_w2_residuals(
    fx$yields, fx$tp,
    maturities = 24, n_pcs = 4, pcs = fx$pcs,
    y1 = fx$y1, y1_lags = 2L, impose_b_zero = TRUE
  )
  # Recover the raw SDF innovation and the complete rows for maturity 24.
  sdf <- compute_sdf_innovations(
