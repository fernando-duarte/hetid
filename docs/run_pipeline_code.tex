...
\author{}
\date{Source inspection: 1 August 2026, 23:30 EDT\\
Commit \texttt{d39c6b9d74f7a74d8f754ffe17447f2d99a6cc39}}
...
This document defines common abbreviations before the detailed terminology reference.
...
guarded body. The six driver-level body guards appear after the table. This rule assigns an explicit
...
Six always or conditionally sourced drivers guard their top-level bodies. Four
...
joint-GMM and residual-dynamics drivers each require \code{log_var_eq} and
\code{set_id_mean_eq}. The conditionally sourced LAD driver again requires all
three upstream objects. Every one of these predicates is true in runner order if
earlier stages succeed. The EGARCH-X driver instead guards on file existence:
it requires the fresh residual-dynamics RDS, and the immediately preceding gate
either writes it or stops, so that predicate is also true when reached. The
always sourced LAD fitted-scale driver is different again. It calls
\src{scripts-paper/config/logvar_estimator_access.R}{paper_logvar_result} with
\code{required=FALSE}, which returns \code{NULL} unless the registry's LAD result
object \code{log_var_eq_lad} exists, so its body runs only when the
dependency-gated LAD map ran. These guards let tests and offline tools source the
same files without running their pipeline bodies. They do not create
hidden production choices.
...
cache provenance. If the acquired data differ, the bootstrap input hash
...
setting changes. The four tracked rows exhaust the top-level plan and decision
...
here asked for. The recorded release tag does a different job. Nothing on disk
carries a tag the check could read, so the tag selects which asset to fetch when
...
Transport failures, draws that return an error instead of a result, may not exceed $\lfloor0.25B\rfloor$. Separately, each
...
Semantic provenance compares 19 of the record's 20 fields: the resampler, sample
...
stage, so it counts as draw code. The runtime hash also includes the whole
