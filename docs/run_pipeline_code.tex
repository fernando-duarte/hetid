...
\author{}
\date{Source inspection: 4 August 2026, 17:30 EDT\\
Commit \texttt{ce9e4adb8ae231e5a0cf25db6b8f43f8be782ff5}}
...
\begin{itemize}
  ...
  \item After the PPML and Harvey tau-zero point fits exist, the runner always
    attempts two residual-diagnostic figures. They compare the standardized log
  ...
  \item A plain invocation calls the \emph{installed} \code{hetid} namespace, not
    ...
    the runtime only if the two agree. The preceding source inspection compared
    the installed namespace with the checkout over the sorted symbol set, on
    names, formal arguments, and deparsed bodies, and found 171 functions on
    both sides with no difference. The last commit changed no file under
    \file{R/}, but this last-commit-only update did not repeat the installed
    namespace comparison, so installed-package parity is currently unverified.
    Deparsing strips source references and enclosing environments, which
    otherwise report a mismatch between identical code. The recorded comparison
    is function-level evidence, not a byte-level attestation of the installed
    build, and reinstalling from a different tree invalidates it.
\end{itemize}
...
estimator, tau-zero residual-diagnostic figures, diagnostics, routes, and
...
residual-diagnostic figures, the joint-null witness diagnostic, the joint-GMM
...
reference style. The stage also prints each series' mean, variance, skewness,
and excess kurtosis beside the corresponding reference moments.
