...
\author{}
\date{Source inspection: 9 August 2026, 09:42 EDT\\
Commit \texttt{6bef02d28281555740cfc030f563df155330866e}}
...
complete input and control interface, exact default route,
...
\begin{itemize}
  \item The production graph names six environment inputs and no command-line
    ...
    also consumes five tracked plan or decision inputs: the mean-specification
    plan, the heteroskedasticity-instrument choice, and the LAD, joint-GMM, and
    EGARCH-X decisions. Both external data inputs
    are frozen by checked-in switches, so a plain invocation reads a tracked
    consumption snapshot and a digest-pinned daily ACM cache. The instrument
    choice names one column of a two-entry registry; the checked-in choice is the
    level of realized five-year-yield volatility. The current LAD decision
  ...
  \item Published mean specification B residualizes each
    ...
    $\beta_2^R=0$. The published lagged-expected-SDF and combined variance shares are
    set-valued.
  ...
  \item After the PPML and Harvey tau-zero point fits exist, the runner always
    attempts two residual-distribution figures. They compare the standardized log
    residuals for PPML, the Harvey estimator, and log-OLS with the
    log-chi-square reference. The stage reuses the accepted PPML and Harvey
    point fits and recomputes only the deterministic log-OLS projection.
  ...
  \item The artifact manifest has 89 artifact records. Final
    conditional-route reconciliation covers the seven conditional records and
    does not certify the 82 required files.
  \item A plain invocation calls the \emph{installed} \code{hetid} namespace, not
    ...
    the runtime only if the two agree. Installed-package parity is
    \emph{unverified}. This trace runs no R, and the installed namespace keeps its
    code in a lazy-load database that only R can read, so the two sides can be
    compared here on metadata alone: both report version 0.4.0, and the installed
    \file{NAMESPACE} is byte-identical to the checkout's. Matching version strings
    and export lists do not establish matching function bodies. The limitation is
    a boundary of this trace, not evidence of a mismatch.
\end{itemize}
...
the paper layer. The module catalog and production entrypoint appear in
\file{scripts-paper/README.md}. Repository guidance also names a root
\file{CLAUDE.md} carrying project rules and standing instructions; that file is
untracked and absent from this checkout, so this trace could not read it.

\subsection{Scope of the trace}

The trace covers every direct top-level effect in
\file{scripts-paper/run_pipeline.R}, including its 49
\code{paper_source_once()} call sites. After prior statements succeed, the
runner reaches 48 call sites unconditionally. It conditionally sources only
...
estimator, tau-zero residual-distribution figures, diagnostics, routes, and
...
$\downarrow$\\[-0.2em]
\fbox{\parbox{0.86\textwidth}{\centering
figures, heteroskedasticity diagnostic battery tables, SDF approximation-error variance
...
in the package cache or stopped the run, so the extractor's own download branch is
unreachable unless that asset disappears between the two steps. &
...
Heteroskedasticity instrument: source-selected &
The contract's \code{instrument$active} names \code{y60_vol}, so the single
heteroskedasticity instrument $Z$ is the realized quarterly volatility of the
five-year yield, which the estimator demeans. &
A source edit to \code{instrument$active} selects any other entry of
\code{PAPER_INSTRUMENT_CHOICES}; the one alternative today is
\code{y60_vol_log}, the natural log of the same column, which the yield-volatility
producer computes whichever entry is active. An unregistered id stops. Switching the active entry
also obliges a regenerated EGARCH-X decision record, because that record binds the
residual-dynamics gate the active instrument produces.\\
...
Run the two-step log-OLS benchmark, PPML, the Harvey estimator, the tau-zero
residual-distribution figures, the joint-null witness diagnostic, the joint-GMM
diagnostic, and the residual-dynamics gate. &
...
except lines 123--125, whose status is \emph{selected path} and source-selected, subject to
...
lines 123--125 row or a guarded body says otherwise, every row is always
...
Validate the package root, define paths and the source-once registry, and
chain-source five preflight modules: runtime primitives, the analysis contract,
the instrument registry, the acquisition controls, and the endpoint-status
contract.\\
...
Produce the daily-change-based quarterly yield-volatility panel, one column per
ACM maturity, and both registered instrument columns: \code{y60_vol}, which the
contract currently selects, and its natural log \code{y60_vol_log}. Stop unless
\code{y60_vol} is positive wherever it is observed.\\
...
Compute \code{var_share} for lagged expected-SDF, SDF-news, and combined blocks.\\
...
78--80 & \file{log_variance/figures/residual_diagnostics/run.R} &
Reuse the accepted PPML and Harvey tau-zero point fits, recompute the fixed
log-OLS projection, and write the residual quantile--quantile and density SVGs.\\
\midrule
84 & \file{log_variance/diagnostics/joint_null/distance_objective.R} &
Define the joint-null objective.\\
85 & \file{log_variance/diagnostics/joint_null/search.R} &
Define its scan, multistart, and polish routines.\\
86 & \file{log_variance/diagnostics/joint_null/stability.R} &
Define crossing-geometry and witness-stability checks.\\
87 & \file{log_variance/diagnostics/joint_null/run.R} &
Run the joint-null witness diagnostic and write its CSV/RDS pair.\\
90 & \file{log_variance/diagnostics/joint_gmm/run.R} &
Validate the decision record and run the currently wired joint-GMM replication diagnostic.\\
97 & \file{log_variance/diagnostics/dynamics/gate_core.R} &
Define the residual-dynamics gate and gate-record builders.\\
98 & \file{log_variance/diagnostics/dynamics/run_gate.R} &
...
\midrule
129 & \file{inference/run_bootstrap_stage.R} &
...
132 & \file{mean_equation/inference/spec_comparison.R} &
...
155 & \file{log_variance/figures/fitted_volatility/run_lad.R} &
...
156 & \file{mean_equation/figures/prepare_region_geometry.R} &
Prepare rescaling, refined boxes, grids, and envelopes for mean-region figures.\\
157 & \file{mean_equation/figures/render_projections.R} &
Render projections with and without the identification-sample OLS benchmark point.\\
158 & \file{mean_equation/figures/render_region_3d.R} &
...
units, and OLS modes.\\
159--164 & \file{mean_equation/diagnostics/heteroskedasticity/render_table.R} &
...
step 0.005; fitted-volatility sweep over relaxation values 0.05, 0.10, 0.20,
0.30, 0.40.\\
...
full 1--120 month grid before any data-construction source module.

The heteroskedasticity instrument is a registry choice rather than a name fixed in
place. \src{scripts-paper/config/instrument_choices.R}{PAPER_INSTRUMENT_CHOICES}
holds one entry per admissible column, each carrying a column name and a
description: \code{y60_vol} and \code{y60_vol_log}.
\code{PAPER_ANALYSIS_CONTRACT$input$instrument} carries only \code{active} and
\code{producer}; \code{active} is \code{y60_vol}, so the selected path is the
realized quarterly volatility of the five-year yield, and \code{y60_vol_log} is an
alternative path present in source but not selected.
\src{scripts-paper/config/instrument_choices.R}{paper_instrument_choice} resolves
\code{active} against the registry and stops on an unregistered id, and
\code{paper_instrument_description()} renders the active entry's description.
\file{scripts-paper/config/paths.R} sources the registry immediately after the
contract, and \file{scripts-paper/config/analysis.R} then defines \code{z_col} as
the active entry's column. The estimator demeans that column.
...
the analysis and reporting contracts, \code{PAPER_SPEC_PLAN}, the instrument
registry and its active choice, the joint-GMM and
...
first call. The installed \code{hetid} version and the installed namespace's
function bodies both enter bootstrap runtime provenance, and so do the bytes of
the checkout's \file{R/} directory. Editing a package source file therefore
invalidates cache reuse without changing what executes.\\
...
these base or recommended R namespaces: \code{graphics}, \code{grDevices},
\code{grid}, \code{parallel}, \code{stats},
...
produce one realized-volatility column per ACM maturity, among them
\code{y60_vol}, and the same producer builds \code{y60_vol_log} unconditionally
for the alternative registry choice. That derivation asserts \code{y60_vol} is
positive wherever observed, so a nonpositive realized volatility stops the run
rather than yielding $-\infty$. The New
...
The runner and its production configuration consume eleven top-level behavior
inputs: six environment inputs and five tracked plan or decision inputs. The
tracked inputs are the mean-specification plan, the instrument choice, and the
LAD, joint-GMM, and EGARCH-X decisions. The separate reset command exposes one
maintenance flag.
Table~\ref{tab:user-controls} lists all twelve without implying that the reset
...
\caption{Complete top-level behavior-input interface: eleven pipeline inputs plus
...
Instrument choice: \code{PAPER_ANALYSIS_CONTRACT$input$instrument$active} &
The tracked value in \file{scripts-paper/config/analysis_contract.R} is
\code{y60_vol}. The allowed ids are the keys of
\code{PAPER_INSTRUMENT_CHOICES} in
\file{scripts-paper/config/instrument_choices.R}, currently \code{y60_vol} and
\code{y60_vol_log}; an unregistered id stops. &
Selects the column \code{z_source()} supplies as the single heteroskedasticity
instrument $Z$. It also selects the rendered instrument description bound to
\code{z_desc}, which no current table or figure reads. It therefore
changes the mean identified set, both log-variance estimators, the
heteroskedasticity diagnostic battery, and unified-bootstrap identity four times over:
through the stage input hash, through the draw-specification hash, which stores
\code{z_col} verbatim, through the analysis contract in the runtime hash,
and through that contract's membership in the bootstrap draw-code manifest.
Switching it also obliges a run of the separate
\file{scripts-paper/tools/regen_egarch_decision.R} entrypoint, because the
checked-in EGARCH-X decision record binds the previous choice's gate.\\
...
The five tracked rows exhaust the top-level plan and decision
...
accounts for every source-fixed control family in \file{config/} and every
protocol authority the production graph reaches. Producer-module label and series
vocabularies, such as the residual-diagnostic estimator list and the
heteroskedasticity test labels, are covered with their stages instead. They lie
outside the draw-code manifest and outside the runtime hash's control objects, so
editing one changes published content without invalidating unified-bootstrap
reuse.
...
policies; ACM, yield-volatility, and consumption inputs; the active instrument
choice id and its producer path; and the
...
\code{input$instrument} now carries only \code{active} and \code{producer}; the
column and description it selects live in the registry row below, and the choice
itself is the supported control listed separately in
Table~\ref{tab:user-controls}.\\
\src{config/instrument_choices.R}{PAPER_INSTRUMENT_CHOICES},
\code{paper_instrument_choice()}, and \code{paper_instrument_description()} &
The registry of admissible heteroskedasticity instruments: one column name and
one description per choice id, plus the resolver that maps the contract's
\code{instrument$active} to that pair and stops on an unregistered id. & This
file is in neither the bootstrap draw-code manifest nor the presentation
manifest, and \code{PAPER_INSTRUMENT_CHOICES} is not one of the control objects
in the runtime hash. Adding a choice therefore invalidates nothing: the new
column is not selected, and only the quarter key and the active column reach the
stage input hash. Switching which choice is active is a different edit, and it
invalidates reuse through the analysis contract, as that row states.\\
...
\file{config/artifact_registry_queries.R},
...
suites, Goldfeld--Quandt settings, the fitted-standard-deviation ratio cutoff,
the rank test's Newey--West bandwidth rule, rejection threshold, and caption
tests. &
...
point sizes, opacity, markers, region fills, residual-density smoothing and
grid size, and quantile--quantile padding. & Changes rendered bytes and
appearance, not estimates or cache validity.\\
\src{config/figure_render_control.R}{PAPER_FIGURE_RENDER_CONTROL} & Projection and
three-dimensional grids, views, region relaxation values, device sizes,
residual-diagnostic canvases, labels, fonts, and layouts. & Changes geometry
evaluation and figure content. Region
relaxation values also change manifest membership; the fitted-volatility sweep
axis instead belongs to the analysis contract. \file{config/tau_grid.R} consumes
the \code{bounds_tau_grid} settings and the contract's figure grid size to build
the shared bounds-by-relaxation-value grid. This file is outside the
...
``unreached'': seven of them run inside the 14. \code{extract_acm_data()} calls
...
it; \code{compute_variance_bound()} calls \code{compute_c_hat()},
\code{compute_k_hat()}, and \code{compute_k2_hat()}; and
\code{build_quadratic_system()}, on the \code{HETID_ASSERT_EQUIV} path alone,
calls \code{compute_identified_set_components()} and
\code{compute_identified_set_quadratic()}. The other 13, among them
...
Not implemented &
The decision schema reserves the flag but rejects any value other than
\code{FALSE} with \code{unsupported_gaussian_gap_restriction}. No Gaussian-gap
solver exists.\\
...
defines \code{mbb_index()}, the production-active circular-MBB
resampler.\\
...
EGARCH-X decision-record regenerator
\file{scripts-paper/tools/regen_egarch_decision.R} &
Separate entrypoint &
\code{Rscript} \code{scripts-paper/tools/regen_egarch_decision.R} reads the
residual-dynamics gate record left by an immediately preceding run and prints the
\file{config/decisions/egarch.R} fields that record would need. It writes nothing
and edits nothing; a human pastes the printed \code{gate_science_sha256},
\code{sample_id}, \code{gate_lag}, \code{gate_alpha}, \code{gate_q},
\code{gate_p}, and \code{gate_verdict}, and separately updates
\code{decided_at_utc} to the printed timestamp. It exists because the checked-in decision
record binds a scientific hash of the active instrument's own gate and the router
refuses a gate that does not match, so switching \code{instrument$active} makes
the checked-in record inapplicable. It stops when the gate artifact is absent and
warns when that artifact is more than thirty minutes old, because the record
carries no instrument identifier of its own. It lives under \file{tools/}, not
bare under \file{scripts-paper/} (the source-topology gate admits exactly two
entrypoints there) and not under \file{validation/} (that directory's own
charter is cross-run TeX-table acceptance only, and this tool reads an RDS
gate cache instead). The runner never calls it.\\
Committed instrument-comparison artifacts
\file{scripts-paper/validation/instrument_comparisons/} &
Separate entrypoint &
Ten generated files under \file{y60_vol_log/}, plus a \file{PROVENANCE.md}, all
tracked. Their basenames repeat manifest basenames, but they are not manifest
records: every artifact path resolves under \file{scripts-paper/output/}, no
production file names this directory, and the runner neither writes nor deletes
it. \file{PROVENANCE.md} records that they came from a disposable clone of an
earlier commit under three uncommitted edits, chief among them
\code{instrument$active} set to \code{y60_vol_log}. They document a robustness
comparison against a nonactive instrument. A plain invocation produces none of
them.\\
Paper test orchestrator \file{scripts-paper/tests/run_tests.R} &
Separate entrypoint &
It launches two structural checks and 39 listed test suites in fresh R
...
the paper test orchestrator. The \file{docs/} tree is gitignored and this file is
tracked on no branch, so no checkout carries it and this trace could not read it.
\file{scripts-paper/README.md} documents the command,
\file{scripts-paper/tests/support/topology_reference_checks.R} exempts its
basename from the stale-reference scan for that reason, and
\file{docs/prompts/orchestrator-prompt-pipeline-run.md} records that it writes its
artifacts under \file{docs/quality-reports/}.\\
\bottomrule
\end{longtable}
\endgroup
...
Before the runner creates directories or deletes conditional artifacts, the
publishing-run guard rejects any replication count other than 10,000 unless
\code{HETID_ALLOW_DRAFT_RUN} is nonempty.
...
and removes the file again if the digest fails. On that path's passing digest it
writes a \code{.meta} sidecar recording the digest, source URL, and retrieval
date; an already-cached asset that matches its pin returns before any write. It
...
so the extractor's own download branch is unreachable unless that asset
disappears between the two steps. Daily ACM yields and their realized
...
The daily-yield analysis stage separately builds the yield-volatility panel, and
the analysis contract selects the heteroskedasticity instrument from that panel's
columns. The bundled \code{hetid::variables} data supply the nominal
asset-return PCs.
...
\subsection{Yield-volatility panel and heteroskedasticity instrument}
...
and defines the square root as the yield-volatility panel. It builds one column
per ACM maturity, \code{y1_vol} through \code{y120_vol}, by appending the
contract's realized-volatility suffix to each package yield-column name. A
quarter with no
observed daily change stays missing instead of collapsing to zero, because a zero
would enter the instrument as a legitimate extreme-low volatility. The stage then
asserts that \code{y60_vol} is positive wherever it is observed and adds
\code{y60_vol_log}, the natural log of that column. The assertion is the log's
precondition: $\log 0$ is $-\infty$ rather than missing, so a zero would survive
the downstream complete-case filter and corrupt the demeaning instead of dropping
the row.

\src{scripts-paper/config/instrument_choices.R}{PAPER_INSTRUMENT_CHOICES}
registers the admissible heteroskedasticity instruments, one entry per admissible
column, both of which this stage computes: \code{y60_vol} and \code{y60_vol_log}.
Each entry carries a column name and a description, stored in the fields
\code{column} and \code{label}. \code{PAPER_ANALYSIS_CONTRACT$input$instrument$active} names
the active entry. The checked-in value is \code{y60_vol}, so a plain invocation
uses the five-year realized-volatility column, and \code{y60_vol_log} is an
alternative path that only a source edit selects.
\src{scripts-paper/config/instrument_choices.R}{paper_instrument_choice} resolves
the active entry and stops on an unregistered id.
\src{scripts-paper/config/analysis.R}{z_col} is that entry's column,
\code{z_source()} returns that column with the quarter key, and \code{z_desc} is
\code{paper_instrument_description()}, the LaTeX rendering of the entry's
description. No production consumer reads \code{z_desc}: the binding exists, but
no current caption or table prints it. The active column supplies the one
heteroskedasticity instrument for all three SDF-news PC scores, and the identification
estimator demeans it on its aligned sample. Switching the active entry also
requires regenerating the checked-in EGARCH-X decision record through the separate
entrypoint \file{scripts-paper/tools/regen_egarch_decision.R}, because that
record binds the residual-dynamics gate the active instrument produces.
...
heteroskedasticity instrument, and three SDF-PC score blocks that the
...
The identified-set driver builds its own frame. It adds the active instrument
column, \code{z_col}, which the checked-in contract resolves to \code{y60_vol};
applies
...
mean specification A second. The same file rejects a plan whose volatility panel
is not the mean panel's published entry; that check is a source-time hard stop,
evaluated while \file{scripts-paper/config/analysis.R} is sourced, before the
runner reaches any stage. The driver retains both fits in
\code{set_id_mean_eq_by_spec} and binds B to the downstream name
\code{set_id_mean_eq}.
...
call the residual $\omega_1$. Their only structural difference is the reduced form
for $Y_2$.
\begin{table}[htbp]
\centering
\small
...
SDF-news PC score block. $\omega_2$ is the specification-specific news block after
...
$\omega_2$ & residuals from regressing each SDF-news PC score on $X$ & raw SDF-news
PC score block $Y_2$\\
...
{estimate_set_id_system}. It demeans $Z$, the active instrument column, computes the package
...
After the estimator constructs $\omega_1$ and the specification-specific $\omega_2$, the
identification problem is
\begin{align*}
  \omega_{1t} &= \omega_{2t}\theta + \varepsilon_{1t},\\
  \omega_{2t} &=
  \begin{cases}
  ...
  \end{cases}
\end{align*}
Here $\omega_1$ is quarterly real consumption growth residualized on $X$. $\omega_2$ is
...
\[
  \left|\operatorname{Corr}
    \left(Z_t,\varepsilon_{1t}\omega_{2it}\right)\right|
  \leq \tau_i
  \left|\operatorname{Corr}\left(Z_t,\omega_{2it}^{2}\right)\right| .
\]
The restriction compares the instrument's correlation with
$\varepsilon_{1t}\omega_{2it}$ against its correlation with $\omega_{2it}^2$. At
...
count. Bars denote column means. The divisor is $T$ rather than $T-1$. For $\omega_{2i}$, let
$\omega_2\odot \omega_{2i}$ denote the matrix whose columns multiply each $\omega_2$ column by
$\omega_{2i}$. The package computes
\begin{align*}
  S_i^{(0)} &= \Varhat(\omega_1\omega_{2i}),&
  \sigma_i^2 &= \Varhat(\omega_{2i}^2),\\
  R_i^{(0)} &= \Covhat(Z,\omega_1\omega_{2i}),&
  R_i^{(1)} &= \Covhat(Z,\omega_2\odot \omega_{2i}),\\
  P_i^{(0)} &= \Covhat(Z,\omega_{2i}^2),&
  S_i^{(1)} &= \Covhat(\omega_1\omega_{2i},\omega_2\odot \omega_{2i}),\\
  &&S_i^{(2)} &= \Varhat(\omega_2\odot \omega_{2i}).
\end{align*}
...
\subsection{Tau-zero point, profile endpoints, and estimated boundedness frontier}
...
coefficient. Published mean specification B makes the lagged-expected-SDF and
combined shares set-valued. Diagnostic mean specification A leaves the lagged
expected-SDF block constant.

\file{scripts-paper/mean_equation/variance_shares/} forms centered divisor-$T$
second-moment blocks for the lagged expected-SDF and SDF-news PC score blocks and
their covariance. The lagged block is the one that enters, because the mean
equation's control block $X$ is the lagged expected-SDF PC score block.
With $B_E(\theta)$ denoting the lagged expected-SDF coefficient block and $V_c$ the
...
SDF-news and lagged expected-SDF PC score blocks. $S_{EN}$ is their cross-moment
matrix.
...
\[
  e_t(\theta)=\omega_{1t}-\omega_{2t}\trans \theta,
  \qquad
...
the first entry in the bounds-rendering registry. The runner computes refined mean
coefficient boxes in the next stage,
\file{scripts-paper/mean_equation/inference/compute_bounds_by_tau.R}, because the
nonlinear estimators consume those exact relaxation-value-keyed boxes.
...
the log-variance set engine, warm-refined mean coefficient box, search seed (the
tau-zero point whenever it is feasible, and otherwise one identical anchor, the
first row of the shared coarsened baseline grid), and
...
size, both replication counts, both block lengths, block rule, seed, RNG
...
runtime hash adds R and platform details; versions of
\code{hetid}, \code{nloptr}, and \code{sandwich}; repository R sources; loaded
namespace functions; BLAS; LAPACK; and scientific controls. The
twentieth field, \code{presentation_sha}, is recorded and format-checked but
compared against itself, so it can never refuse reuse and the provenance record
stores it for audit only. A presentation helper that changes a
...
\subsection{Tau-zero residual-distribution figures}

Immediately after the Harvey analysis stage,
\file{scripts-paper/log_variance/figures/residual_diagnostics/run.R} constructs
one standardized log-residual series for the two-step log-OLS benchmark, PPML,
and the Harvey estimator. At the accepted tau-zero mean news coefficient
$\widehat\theta_0$, define
\[
  \xi_{a,t}
  =\log\!\left(e_t(\widehat\theta_0)^2\right)
   -R_t\trans\widehat\eta_a(\widehat\theta_0)
  =\log\!\left(
    \frac{e_t(\widehat\theta_0)^2}{\widehat\mu_{a,t}}
  \right),
  \qquad
  \widehat\mu_{a,t}=\exp\!\left(R_t\trans\widehat\eta_a\right),
\]
for $a\in\{\text{log-OLS},\text{PPML},\text{Harvey}\}$. For PPML and the
Harvey estimator, $\widehat\mu_{a,t}$ is the fitted conditional variance. For
log-OLS, it is the exponentiated fitted conditional mean of the log squared
residual and carries no retransformation correction.

The stage reads the PPML and Harvey coefficients from their unique accepted
tau-zero point fits and never refits either estimator. It recomputes the
closed-form log-OLS projection at the same $\widehat\theta_0$. It stops if the
mean point is missing, either primary estimator lacks one unique accepted point
fit with five finite coefficients, a structural mean residual is exactly zero,
or any standardized log residual is nonfinite.

Under conditionally normal structural mean residuals and a correctly fitted
conditional variance, $\xi_{a,t}$ follows $\log\chi_1^2$. The first required SVG
plots sorted empirical values against the corresponding theoretical quantiles,
using common axis limits with three-percent padding and a square fixed-aspect
canvas. The second plots estimator-specific kernel densities against the exact
reference density
\[
  f_{\log\chi_1^2}(x)
  =\sqrt{\frac{e^x}{2\pi}}\exp\!\left(-\frac{e^x}{2}\right).
\]
The density curves use smoothing adjustment 1.8, and the reference curve uses
512 grid points. Both figures share one estimator palette and one neutral
reference style. The stage also prints each series's mean, variance, skewness,
and excess kurtosis, followed by one shared reference value, the
$\log\chi_1^2$ mean.
...
treat stored endpoint status as data. Only \code{bounded} status yields a finite
displayed endpoint. The mean and variance-share cells suppress every
non-\code{bounded} status outright; the log-variance cells suppress
\code{unreliable}, and an \code{unbounded} side always carries an infinite value
...
of the typical quarterly move in the one-quarter yield. A further assertion requires that move to
...
acceptance check must establish existence and freshness for the 82 required
...
The expanded artifact manifest contains 89 unique artifact records: 55 literal
records, 18 fitted-volatility-sweep records, and 16 generated mean-region
records. Of these, 82 are \code{required}, three \code{conditional_lad}, and four
...
\centering
\small
\caption{Counts for all 89 expanded artifact-manifest records by output group.
...
manifest contains 82 required records, three records conditional on LAD, and
...
with 48 of the 89 artifact records.
...
\begin{itemize}
  \item \code{required} means the manifest expects the record regardless of a
    ...
    sweep paths. No final gate checks all 82 required paths.
...
\begin{landscape}
\subsection{The 55 literal records}

The 55 literal artifact records assign one ID to each manifested file. A
...
\caption{The 55 literal artifact records. Group/basename paths are relative to
...
\code{residual_qq_figure} & \file{figures/log_var_eq_residual_qq.svg} & \file{log_variance/figures/residual_diagnostics/run.R} & R\\
\code{residual_density_figure} & \file{figures/log_var_eq_residual_density.svg} & same residual-distribution producer & R\\
...
tally over these 55 records is 48 \code{required}, three \code{conditional_lad},
and four \code{conditional_egarch}; the 34 generated records are all
\code{required}, which is how the manifest reaches 82 required of 89.

Five of the 89 records have no producer that resolves them. Four are the EGARCH
...
fifth record, \code{descriptive_report_pdf}, differs
entirely: the descriptive producer compiles the wrapper TeX, and \code{latexmk}
writes the PDF beside it, so nothing ever addresses the file through its
...
The other 63 artifact records have blank family and variant fields and are
...
comparator, the EGARCH-X decision-record regenerator, and the untracked
\file{docs/quality-check.R} are separate entrypoints. Typed writer
...
Contract-ownership checks are separate from the topology checks and run as the
second structural check. They deparse every non-test R file under
\file{scripts-paper/} and reject a forbidden construct outside the one file that
owns it, reject retired module paths, reject banned prose and statistic literals,
require every fit-status predicate to route through \code{LOGVAR_FIT_STATUS},
assert positively that exactly one file defines
\code{quadratic_point_feasible}, and hold the engine oracle to its frozen
verbatim form. They establish ownership and absence, not the correctness of what
the owner computes.
...
structure, rendered layout, and non-table artifacts. The topology, ownership,
round-trip, readback, and projection checks establish only their named properties.
...
existence for all 82 required paths. Final conditional-route reconciliation may
...
\caption{Canonical scientific terms and their current code names. The first column
gives the one prose name this document uses for the concept. The second lists the
source identifiers, comment phrases, and mathematical symbols that denote it. The
third says why those names denote one concept. The prose uses only the canonical
term; the code names appear here so a reader can match prose to source.
...
lagged expected-SDF PC score block & \code{lag_expected_sdf_pc}; the
mean equation's control block \code{x_cols}; variance-share code and comments
that say expected-SDF block or \code{s_e} & A fresh PCA
...
that score block. It is the block the variance shares use, even though the
variance-share code calls it the expected-SDF block.\\
...
code comments saying $\omega_2$ component or news component & The current paper
application supplies the three columns of the SDF-news PC score block as
$Y_2$. Generic identification code calls a $\omega_2$ column a component; this
...
heteroskedasticity instrument &
\code{PAPER_INSTRUMENT_CHOICES}, \code{paper_instrument_choice()}, \code{z_col},
\code{z_source()}, \code{y60_vol}, mathematical $Z$, code comments saying
heteroskedasticity driver or identification instrument, and the package parameter
\code{pcs} of \code{compute_identification_moments()} & The yield-volatility panel
contains one column per ACM maturity plus \code{y60_vol_log}. The instrument
registry lists the admissible columns; the analysis contract's
\code{instrument$active} names one of them, and \code{z_col} is that entry's
column. The checked-in active entry is \code{y60_vol}, the five-year column,
which the estimator demeans. The package parameter is
named \code{pcs} because the package's general interface accepts a matrix of
instruments; the paper passes a single demeaned column,
not principal components of anything.\\
...
is the three-vector multiplying $\omega_2$ in
$e_t(\theta)=\omega_{1t}-\omega_{2t}^{\top}\theta$. The log-variance code commonly calls
...
tau-zero residual-distribution figures &
\code{log_var_eq_residual_diagnostics},
\code{LOGVAR_RESID_DIAG_ESTIMATORS}, \code{residual_qq_figure}, and
\code{residual_density_figure} & These are the quantile--quantile and density
comparisons of three estimator-specific standardized log-residual series. They
are published figures, not the Ljung--Box residual-dynamics gate.\\
...
\caption{Canonical operational terms and their current code names. Operational terms
name execution, artifact, and validation objects rather than scientific estimands. The
first column gives the canonical prose name, the second the source identifiers and
comment phrases that denote it, and the third the meaning and the rule for mapping the
two.
...
instrument registry & \code{PAPER_INSTRUMENT_CHOICES} and its resolver
\code{paper_instrument_choice()} & A two-entry list of admissible instrument
columns, one \code{column} and one \code{label} per choice id. It is not the
selected heteroskedasticity instrument: the registry enumerates what may be
chosen, and the analysis contract's \code{instrument$active} names the one in
force.\\
...
\caption{Similar names that denote different concepts. Each row lists names a reader
could take for one object and states why they are separate. Collapsing any pair in a
row would misstate what the source computes.\label{tab:false-friends}}\\
\toprule
...
registry, instrument registry, artifact manifest, LaTeX publication registry, and
LaTeX label registry & The source-once
...
inputs. The instrument registry stores admissible instrument columns. The
artifact manifest stores artifact metadata records. The LaTeX
publication registry stores publication triples. The LaTeX label registry maps
artifact IDs to cross-reference label strings.\\
...
Instrument choice id, instrument column, and instrument description & The choice
id is the registry key that \code{PAPER_ANALYSIS_CONTRACT$input$instrument$active}
names. The column is the \code{yield_vol} column that key resolves to, which
\code{z_col} holds. The description is the entry's \code{label} field.
\code{paper_instrument_choice()} returns the whole entry, and
\code{paper_instrument_description()} returns only its LaTeX rendering. Every
registered id currently equals its own column name, a naming
...
mean identified set.\\
$\omega_1$, $e_t(\theta)$, and $u_t$ & $\omega_1$ is quarterly real consumption growth
residualized on $X$. The residual $e_t(\theta)=\omega_{1t}-\omega_{2t}^{\top}\theta$
...
\file{scripts-paper/config/analysis_contract.R};
\file{scripts-paper/config/instrument_choices.R};
...
\file{scripts-paper/validation/instrument_comparisons/};
\file{scripts-paper/reset_pipeline_state.R};
\file{docs/quality-check.R} (untracked) &
\ref{sec:outside-runner}\\
Analysis contract and policies & \file{scripts-paper/config/analysis_contract.R}; \file{scripts-paper/config/analysis.R}; \file{scripts-paper/config/instrument_choices.R} & \ref{sec:preflight}\\
...
Tau-zero residual-distribution figures &
\file{scripts-paper/log_variance/figures/residual_diagnostics/};
\file{scripts-paper/config/figure_rendering.R} & \ref{sec:publication}\\
