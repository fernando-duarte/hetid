...
separately. Realized yield volatility serves as the mean-equation instrument. The resulting
restrictions define the mean identified set of admissible coefficients. Four conditional-scale
estimator mappings carry that set into conditional-scale estimates, which describe the size of
unexplained consumption growth. A 10,000-draw circular moving-block bootstrap resamples consecutive
quarter blocks for inference, and two plug-in calculations bound approximation-error variance. Part
I defines the current configuration, comparisons, and sensitivity checks. Part II catalogues
permitted modifications and unavailable extensions. Exact repository-independent reproduction
remains uncertified because some inputs lack immutable public identities, some principal-component
constructions lack absolute-sign rules, and some release-specific or numerical-library choices lack
complete cross-language algorithms.
\end{abstract}
...
selected external numerical reference passes the exact-release gate. The residual-dynamics branch
continues only when its fresh diagnostic agrees exactly with the selected decision.
...
\begin{enumerate}
\item The monthly Treasury input contains U.S. zero-coupon yields and term premia by maturity. First
use the recognized user-supplied Adrian--Crump--Moench (ACM) monthly 1--120 month replication asset when it exists. Its
release or vintage is not recorded or verified, so this higher-precedence arm has no immutable
external identity. Otherwise use the public \emph{ACM Term Premium 2026-06} monthly 1--120 month
asset, which contains 780 observations from 30 June 1961 through 29 May 2026, at
...
billions of chained 2017 dollars, seasonally adjusted at an annual rate. The public series page is
mutable, and the supplied snapshot has no recorded immutable Federal Reserve Economic Data (FRED) or Bureau of Economic Analysis
...
The public June 2026 monthly fallback supplies 780 observations. The July 2026 daily panel begins on 14 June 1961 and
...
coordinate \(k\) before assembling the inequalities and stop otherwise. The quadratic inequalities
...
row is not the sum of the two block rows, or to a positive-slack range column. Skip the adjustment
...
PC-score system is a separate reported diagnostic. Select the Anscombe regime separately for the
two systems.
...
observation count. Its count and span are determined by the selected monthly ACM arm and the
complete-row joins; do not fix them before the inputs have been selected.
...
\emph{nesting rule} requires each attained numerical range not to shrink as the slack rises, and the
...
coefficient vector does not count again toward the new-evaluation cap. The primary PPML
...
the same at every display slack. Require the selected conditional-scale design to have numerical
column rank five; stop before Harvey QML if it does not. Treat the three self-test outcomes as
...
\(\rho_H\) is a certified positive facet when a dual lower bound confirms it. A dual lower bound is
no larger than the minimum on that facet. An infeasible facet is certified infeasible when a phase-I
bound confirms it. A phase-I bound is the matching bound for the smallest constraint violation.
Every other result is unresolved. The Harvey recession-classification certificate subsection of Part~II states those bounds, their
...
20,000 new mapping evaluations at each slack. Then run a separate \emph{Harvey sensitivity audit}
...
For descriptive sensitivity, collect the structural SDF-news coefficient vectors that attain the
...
each vector's dimension and finiteness. Exclude a
vector, while stating the reason, when it produces a zero structural mean residual or a nonfinite
log-OLS residual. Otherwise calculate its Ljung--Box p-values at lags 1, 4, and 8. These sensitivity
values provide context only and never change the zero-slack verdict.
...
The current LAD decision is approved and requires release 6.1 of the selected median-regression
numerical reference. If that exact release is available, perform every LAD calculation in this
...
endpoint state. LAD supplies attained numerical ranges, a table, and figures of the conditional median
...
The current publication calculation accepts no other draw count.
...
tables may display half-infinite attained numerical ranges and their associated half-infinite
...
one conditioning on the SDF-news principal components and the other on the SDF-news reduced-form residuals;
...
\item Produce figures of attained numerical conditional-scale coefficient ranges for log-OLS, PPML, Harvey
...
sensitivity, and the LAD one-sided closure limits. Describe the LAD limits
...
unreliable endpoint-state rule. Permit 80,000 new mapping evaluations at each slack. Convert the fitted log scale to a standard deviation with
...
new-evaluation cap, or containment check. Prepending a feasible zero-slack point already places its
...
half-infinite attained numerical range and, when defined, its associated half-infinite confidence interval.
...
entries and block total are all finite. Leave the positive-slack attained-numerical-range columns
...
A scientific reproduction is accepted only if the selected inputs satisfy the identity and coverage
...
the named public fallback; the frozen consumption series lacks an immutable public vintage identity;
and the supplied return principal components lack a public, immutable source and complete definitions
...
\begin{enumerate}
\item Resolve and verify the monthly and daily Treasury inputs, the frozen consumption series, and
the supplied return principal components under the input rules of Section~\ref{sec:inputs}. Stop if
any required identity, coverage, or convention cannot be established.
...
\item Construct quarterly consumption growth, realized yield volatility, and the centered five-year
volatility instrument.
\item Date the supplied return principal components, shift them forward one quarter, align all
series, and apply only the demeaning transformation stated above.
...
\item Fit the preliminary and benchmark mean regressions. Form the identification sample, estimate
...
ranges. Evaluate the mean-equation identification diagnostics and the reported-versus-comparison
specification check.
\item Form the conditional-scale sample and calculate the log-OLS, PPML, and Harvey QML estimator
...
\item Verify the selected median-regression numerical reference release 6.1, then calculate the LAD
...
\item Construct the mean-identified-set projections, attained numerical conditional-scale coefficient ranges, fitted
...
already calculated degenerate range. None changes the mean or scale estimands.
\end{description}

\section{Conditional-scale estimators and inference}

\begin{description}[style=nextline]
\item[LAD route decision.]
\emph{Reachability: Executable full-route modification. Request handling: Supported and unselected.}
Approve, decline, or leave unanswered the LAD calculation. Approval requires the selected exact
numerical-reference release; failure of that dependency stops the branch. Decline or no answer
removes LAD scientific results without changing the other estimators.
...
\emph{Reachability: Executable full-route modification. Request handling: Not exposed.}
Change lattice density and thinning, traversal order, start counts and separation, cold-start audits,
endpoint agreement, nesting, point containment, or return-score centering tolerances. These choices
...
primary, sensitivity, coverage, bootstrap, and fitted-scale searches. The attained endpoints,
...
cone checks, or cold-start audits. Fit acceptance, nonuniqueness evidence, closure classifications,
...
\emph{Reachability: Executable full-route modification. Request handling: Supported and unselected.}
For PPML choose naive, heteroskedasticity-consistent, or heteroskedasticity-and-autocorrelation-
...
A generic per-side diagnostic is recognized, but no current estimator defines the diagnostic
quantity and no scientific result depends on it.
...
suites at evaluation time. Only diagnostic rows and interpretations change.
...
After a rejecting gate, choose ladder-consistent approved, declined, unanswered, or not-asked
decisions for the estimand and dependency. The decision sequence changes terminal route status, but even full approval
reaches no estimator.
...
\emph{Reachability: Executable full-route modification. Request handling: Supported and unselected.}
Choose a nominal level in \((0,1/2)\), a minimum valid-draw share in \((0,1]\), a stability share and
...
\emph{Reachability: Executable component modification. Request handling: Supported and unselected.}
Withdraw attained-range, table, set-bootstrap, or fitted-scale capability from an estimator. The
...
The catalogue contains 88 entries and is exhaustive for deviations recognized by the reproduction contract.
A future modification is a valid alternative reproduction
