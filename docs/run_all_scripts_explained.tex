...
\emph{This 18~July 2026 description is superseded.} On 20~July 2026 a full
run of the current branch completes end to end. The structural-table caption
that halted the 18-July runner no longer exists---its caption and notes were
moved out of the generated fragment (now a bare tabular) and the former
caption and inference-note modules were deleted---and the positive-slack
mean sets are now certified \texttt{bounded} rather than \texttt{unreliable}.
With $N=256$, $\widehat\tau^\ast=0.415$; the three SDF-news loadings have
bounded sets, two excluding zero at $\tau=0.05$; the bootstrap
$\tau^\ast$ range is $[0.157,\,0.493]$, bounded at baseline in 98\% of
draws. The log-variance engine therefore runs: PPML, Harvey, and LAD maps,
the joint-null and joint-GMM diagnostics, fitted-volatility envelopes, and
the mean-region figures are all produced, and the lag-4 dynamics screen's
non-rejection stops the EGARCH workstream as a complete deliverable. The
still-accurate architecture below (source graph, data construction, model,
and Lewbel system) is retained; the obsolete positive-slack
``unreliable'' policy, the typed tau-star ``unresolved'' status, and the
hard-stop subsections have been removed because they no longer describe the
code.
