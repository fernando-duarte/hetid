...
$\theta=\Cov(Z,\omega_1\omega_2)/\Cov(Z,\omega_2^2)$. & Needs heteroskedastic relevance of the first-stage residual. With one instrument it is just identified, so overidentification tests are unavailable. \\
\addlinespace
...
\addlinespace
Lee--Lewbel--Schennach--Zhang plus an external instrument & Add a conventional IV moment such as $\E[Z(\omega_1-\theta \omega_2)]=0$ to the internal non-Gaussian GMM moments. & Usually point identification, often overidentification, with a direct comparison between the external-IV and internal-moment estimates. & If the two information sources conflict, the overidentifying restrictions can reject but cannot by themselves say which assumption failed. \\
\addlinespace
Variance-regime / Rigobon-style identification & A scalar state or instrument $Z$ partitions the sample into variance regimes; the structural impact matrix is fixed while shock variances change nonproportionally. & In a square two-shock representation, the impact ratio for the labeled $\omega_2$ shock can be identified from regime covariance matrices; in the one-changing-shock case it is a simple covariance-difference ratio. & This is an alternative square-shock model, not the three-shock common-factor model. Needs a credible regime/state and shock labeling. \\
\addlinespace
...
representation and labels one shock as the shock whose effect on $\omega_1$ per unit
of $\omega_2$ is the desired coefficient.
...
\begin{align}
\omega_{1,t+1} &:= Y_{1,t+1}-X_t^\T\beta_1^R,\qquad
\beta_1^R:=\bigl(\E[X_tX_t^\T]\bigr)^{-1}\E[X_tY_{1,t+1}],\label{eq:W1}\\
\omega_{2,t+1} &:= Y_{2,t+1}-X_t^\T\beta_2^R,
\qquad
...
system is
\begin{equation}
\omega_{1,t+1}=\theta \omega_{2,t+1}+\varepsilon_{1,t+1},
\qquad
...
\begin{equation}
H_{t+1}:=(Z_{t+1}-\E Z_{t+1})\omega_{2,t+1}
\label{eq:lewbel-instrument}
\end{equation}
is a valid constructed instrument for $\omega_{2,t+1}$ in the residualized equation.
Indeed,
\begin{align}
\E[H_{t+1}\varepsilon_{1,t+1}]&=
\Cov(Z_{t+1},\varepsilon_{1,t+1}\varepsilon_{2,t+1})=0,\label{eq:lewbel-valid}\\
\E[H_{t+1}\omega_{2,t+1}]&=\Cov(Z_{t+1},\varepsilon_{2,t+1}^2)\ne0.\label{eq:lewbel-relevance}
\end{align}
In the scalar residualized system this yields the closed-form population
coefficient
\begin{equation}
\theta=
\frac{\Cov(Z_{t+1},\omega_{1,t+1}\omega_{2,t+1})}
     {\Cov(Z_{t+1},\omega_{2,t+1}^{2})}.
...
\begin{align}
L&:=\Cov(Z_{t+1},\omega_{1,t+1}\omega_{2,t+1}),&
Q&:=\Cov(Z_{t+1},\omega_{2,t+1}^2),\label{eq:LQ}\\
S_0&:=\Var(\omega_{1,t+1}\omega_{2,t+1}),&
S_1&:=\Cov(\omega_{2,t+1}^2,\omega_{1,t+1}\omega_{2,t+1}),&
S_2&:=\Var(\omega_{2,t+1}^2).
\label{eq:S012}
\end{align}
...
$X_t$ and $(Z_{t+1}-\bar Z)\widehat \omega_{2,t+1}$ as instruments. Heteroskedasticity-
...
\begin{equation}
\omega_{2,t+1}=u_{t+1}+v_{t+1},
\qquad
\omega_{1,t+1}=\theta \omega_{2,t+1}+\beta_u u_{t+1}+r_{t+1}.
\label{eq:lsz-raw}
\end{equation}
Equivalently,
\begin{equation}
\omega_{1,t+1}=\alpha u_{t+1}+\theta v_{t+1}+r_{t+1},
\qquad
\alpha:=\theta+\beta_u.
\label{eq:alpha-def}
\end{equation}
...
Let $\Phi_{\omega_2,\omega_1}$ be the log characteristic function of
$(\omega_{2,t+1},\omega_{1,t+1})$, and let $\Phi_{\omega_2}$ be the log characteristic function of
$\omega_{2,t+1}$. Define joint cumulants by
\begin{equation}
\Phi_{\omega_2,\omega_1}^{k,l}:=
\left.
\frac{\partial^{k+l}\Phi_{\omega_2,\omega_1}(\zeta,\xi)}
     {i^{k+l}\partial\zeta^k\partial\xi^l}
\right|_{\zeta=0,\xi=0},
\qquad
\Phi_{\omega_2}^{k}:=
\left.
\frac{\partial^{k}\Phi_{\omega_2}(\zeta)}
     {i^{k}\partial\zeta^k}
\right|_{\zeta=0}.
\label{eq:cumulants}
\end{equation}
For ordinary intuition, $\Phi_{\omega_2}^2$ is the variance of $\omega_2$, $\Phi_{\omega_2}^3$
...
\begin{align}
\Phi_{\omega_2}^{3+p} &= \kappa_u^{3+p}+\kappa_v^{3+p},\label{eq:kappa1}\\
\Phi_{\omega_2,\omega_1}^{2+p,1} &= \alpha\kappa_u^{3+p}+\theta\kappa_v^{3+p},\label{eq:kappa2}\\
\Phi_{\omega_2,\omega_1}^{1+p,2} &= \alpha^2\kappa_u^{3+p}+\theta^2\kappa_v^{3+p},
\label{eq:kappa3}
\end{align}
...
\begin{equation}
M_p(\alpha,\theta):=
\Phi_{\omega_2,\omega_1}^{1+p,2}-\alpha^2\Phi_{\omega_2}^{3+p}
-(\theta+\alpha)\left(\Phi_{\omega_2,\omega_1}^{2+p,1}-\alpha\Phi_{\omega_2}^{3+p}\right)=0.
\label{eq:Mp}
\end{equation}
...
\begin{equation}
\Phi_{\omega_2}^{3+\widetilde q}\,\Phi_{\omega_2,\omega_1}^{2+q,1}
\ne
\Phi_{\omega_2}^{3+q}\,\Phi_{\omega_2,\omega_1}^{2+\widetilde q,1}.
...
\begin{align}
\sigma_u^2 &=
\frac{\Cov(\omega_1,\omega_2)-\theta\Var(\omega_2)}{\alpha-\theta},
\label{eq:varu}\\
\sigma_v^2 &= \Var(\omega_2)-\sigma_u^2,
\label{eq:varv}\\
\sigma_r^2 &= \Var(\omega_1)-\alpha^2\sigma_u^2-\theta^2\sigma_v^2.
\label{eq:varr}
\end{align}
...
exclusion condition
\begin{equation}
\E\left[Z_{t+1}\{\omega_{1,t+1}-\theta \omega_{2,t+1}\}\right]=0,
...
based only on the covariance matrix of $(\omega_2,\omega_1)$. The formulas below describe
that implemented bound.

Assume first that $\beta_u=\alpha-\theta>0$. Define
\begin{equation}
B_0:=\frac{\E[\omega_{1,t+1}\omega_{2,t+1}]}{\E[\omega_{2,t+1}^2]},
\qquad
D_0:=
\frac{\E[\omega_{1,t+1}^2]\E[\omega_{2,t+1}^2]-\{\E[\omega_{1,t+1}\omega_{2,t+1}]\}^2}
     {\{\E[\omega_{2,t+1}^2]\}^2}.
\label{eq:B0D0}
\end{equation}
If the residuals are mean zero, $B_0$ is simply the population OLS slope from
regressing $\omega_1$ on $\omega_2$, and $D_0$ is the residual variance from that regression
scaled by $\Var(\omega_2)$.
...
If instead $\beta_u<0$, apply the positive-loading result to $-\omega_2$. The order
...
\begin{equation}
U_{t+1}:=\begin{bmatrix}\omega_{2,t+1}\\ \omega_{1,t+1}\end{bmatrix}
=B\eta_{t+1},
\qquad
...
column $j$ is interpreted as the shock that moves $\omega_2$, then
\begin{equation}
...
to interpret the selected shock as the exogenous variation in $\omega_2$ whose causal
impact on $\omega_1$ is sought. Shock labeling is therefore central.
...
order. Once a column is labeled as the $\omega_2$ shock, $\theta_j=b_{2j}/b_{1j}$ is
identified.
...
$U=(\omega_2,\omega_1)^\T$, the impact ratio is
\begin{equation}
\theta_j=
\frac{\Cov(\omega_2,\omega_1\mid Z=1)-\Cov(\omega_2,\omega_1\mid Z=0)}
     {\Var(\omega_2\mid Z=1)-\Var(\omega_2\mid Z=0)}.
...
\item \textbf{External IV.} If $\E[Z\varepsilon_1]=0$ and $\E[Z\omega_2]\ne0$, then
$Z$ directly identifies $\theta$ by conventional IV:
\begin{equation}
\theta_{\text{IV}}=\frac{\E[Z\omega_1]}{\E[Z\omega_2]}.
\label{eq:directiv}
\end{equation}
...
\eqref{eq:lewbel-exact-assump}, then $(Z-\E Z)\omega_2$ is the constructed
instrument. Here the relevant covariance is $\Cov(Z,\omega_2^2)$, not $\E[Z\omega_2]$.
...
If $\Cov(Z,\omega_2^2)$ is near zero, the constructed instrument is weak. Standard IV
...
credible, use Lewbel.} Check whether $\Cov(Z,\omega_2^2)$ is meaningfully nonzero. If
...
\end{equation}
Let $\omega_1$ and $\omega_2$ be the residuals from regressions of $Y_1$ and $Y_2$ on
$X$.  Define the sample analogues of
\begin{align*}
C_{12,Z}&=\Cov(\omega_1\omega_2,Z), & C_{22,Z}&=\Cov(\omega_2^2,Z),\\
V_{12}&=\Var(\omega_1\omega_2), & V_{22}&=\Var(\omega_2^2),\\
C_{12,22}&=\Cov(\omega_1\omega_2,\omega_2^2).
\end{align*}
...
\begin{enumerate}
    ...
    \item in each draw, recompute $\omega_1^*,\omega_2^*$, all covariance terms, and the
...
\begin{enumerate}
    \item the OLS slope of $\omega_1$ on $\omega_2$;
    ...
    $\widehat{\Cov}(Z,\omega_2^2)$ and a heteroskedasticity test;
